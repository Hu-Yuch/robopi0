...
from src.model.latent_cache import LatentCache
from src.model.vla.Robo_mixture import Mixture
...
def forward_mixture_layers(
    ...
    latent_caches: dict[LatentCache] = {},
...
def forward_mixture_attn(
    ...
    for name in active_mixture_names:
        ...
        #         "forward_rotary_emb_3d", layer_idx, query_states, position_ids_all[name]
        #     )
        # else:
        #     rope_cos, rope_sin = mixtures[name].attn_func(
        #         "forward_rotary_emb", layer_idx, query_states, position_ids_all[name]
        #     )
        rope_cos, rope_sin = position_ids_all[name]
        ...
        flag_to_cache_latent = (name in latent_caches and not latent_caches[name].has_item(layer_idx)) or cache_mode == "append"
        ...
        #flag_calc_new_latent = not flag_cached_mixture or cache_mode == "append"
        if flag_calc_new_kv:
            ...
            if flag_to_cache_latent:
                latent_caches[name].update(
                    hidden_states,
                    layer_idx,
                )
        ...
        query_states_all[name] = query_states
        #print('name:',name,'query_states', query_states.shape)
    ...
    for name in key_states_all:
        if name in mixtures:
            ...
            key_states, value_states = mixtures['action'].attn_func(
                "repeat_kv",
                layer_idx,
                key_states_all[name],
                value_states_all[name],
...
class JointModel(nn.Module):
    ...
    def build_vlm_caches(self):
        return {'vlm': KVCache()}

    def build_latent_caches(self):
        return {name: LatentCache() for name in self.cache_latent_names}
    ...
    def _gradient_checkpointing_func(
        ...
        position_ids_all: dict[torch.LongTensor],
        embeds_all: dict[torch.FloatTensor],
        time_cond: Optional[torch.FloatTensor],
        layer_idx: int,
        final_layer_post_attn_skip_names: Tuple[str, ...],
        kv_caches: dict[KVCache],
        latent_caches: dict[LatentCache],
        cache_mode: str,
    ) -> dict[torch.FloatTensor]:
    ...
    def forward(
        ...
        kv_caches: dict[KVCache] = {},
        latent_caches: dict[KVCache] = {},
        cache_mode: str = "append_non_active",
        return_caches: bool = False,
        return_latent_caches: bool = False,
        ...
        # [Batch_Size, Seq_Len, Hidden_Size]
        for name in active_mixture_names:
            position_ids_all[name] = self.mixtures[name].rotary_emb(embeds_all[name], position_ids_all[name])

        # layers
        for layer_idx in range(self.num_hidden_layers):
            if use_gradient_checkpointing and self.training:
                embeds_all = checkpoint(
        ...
        if return_caches:
            if return_latent_caches:
                return hidden_states_all, kv_caches, latent_caches
            else:
