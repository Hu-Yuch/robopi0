...
class PiZeroRobo(nn.Module, NoSyncBase):
    ...
    def __init__(self, cfg, use_ddp: bool = False):
        ...
        self.cfg_pretrained = AutoConfig.from_pretrained("BAAI/RoboBrain")
    ...
    def trainable_vlm_parameters(self):
        if self.use_3D:
            return (
                list(self.robobrain.vision_tower.parameters())
                + list(self.robobrain.multi_modal_projector.parameters())
    ...
    def trainable_qwen_parameters(self):
        qwen_parameters = []
        for name, param in self.robobrain.language_model.named_parameters():
    ...
    def _check_qwen_unused_parameter_by_name(self, name: str) -> bool:
        ...
        if (
            f"{last_hidden_layer_index}.post_attention_layernorm" in name
    ...
    def freeze_unused_weights(self):
        ...
        self.robobrain.language_model.model.embed_tokens.weight.requires_grad = False
        self.robobrain.language_model.lm_head.weight.requires_grad = False
        for name, param in self.robobrain.language_model.named_parameters():
    ...
    def split_full_mask_into_submasks(
        ...
        #print('max_image_text_tokens', max_image_text_tokens)
        #print('causal_mask', causal_mask.shape)
        proprio_action_mask = causal_mask[..., max_image_text_tokens :, :]
        return image_text_mask, proprio_action_mask
        ...
        for image_idx, image_feature in enumerate(image_features):
            if image_feature.shape[0] > 1:
                ...
                num_patch_height, num_patch_width = get_anyres_image_grid_shape(
                    ...
                    self.cfg_pretrained.image_grid_pinpoints,
                    self.cfg_pretrained.vision_config.image_size,
                )
    ...
    def infer_action(
        ...
        inputs['image_sizes'] = [[384,384]]*pixel_values.shape[0]
        vlm_ouput = self.robobrain(
            **inputs
        )
        kv_caches = self.joint_model.build_vlm_caches()
        for i in range(len(vlm_ouput['past_key_values'])):
            kv_caches['vlm'].update(vlm_ouput['past_key_values'][i][0], vlm_ouput['past_key_values'][i][1], i)
    ...
    def forward(
        ...
        # text tokens + image tokens
        #_, position_ids, attention_mask, past_key_values, inputs_embeds, new_labels = self.prepare_inputs_labels_for_multimodal(input_ids, vlm_position_ids, causal_mask, None, None, pixel_values, modalities=["image"], image_sizes=None)
        ...
        inputs['attention_mask'] = causal_mask
        inputs['image_sizes'] = [[384,384]]*pixel_values.shape[0]
        print('start forward robobrain')
        vlm_ouput = self.robobrain(
            **inputs
        )
        print('end forward robobrain')
        kv_caches = self.joint_model.build_vlm_caches()
        for i in range(len(vlm_ouput['past_key_values'])):
            kv_caches['vlm'].update(vlm_ouput['past_key_values'][i][0], vlm_ouput['past_key_values'][i][1], i)
        ...
        action_embeds = self.joint_model(
            attention_mask=proprio_action_mask,
            position_ids_all={
                "proprio": proprio_position_ids,
            ...
            time_cond=time_cond,
            kv_caches=kv_caches,  # no caching during training
            cache_mode="append_non_active"
        )["action"]
        print('end_forward_joint_model')
        # [Batch_Size, Horizon_Steps, Action_Dim]
