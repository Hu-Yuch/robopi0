...
class PiZero3D(nn.Module, NoSyncBase):
    ...
    def _forward_siglip_and_text_embedding(
        ...
        with torch.no_grad():
            ...
            # 计算相机焦距
            if isinstance(camera['static_cam_height'], (int, float)):
                cam_height = torch.tensor(camera['static_cam_height'], device=device)
            else:
                cam_height = torch.from_numpy(camera['static_cam_height']).to(device)
                
            if isinstance(camera['static_cam_fov'], (int, float)):
                cam_fov = torch.tensor(camera['static_cam_fov'], device=device)
            else:
                cam_fov = torch.from_numpy(camera['static_cam_fov']).to(device)
                
            foc = cam_height / (2 * torch.tan(cam_fov / 2))
    ...
    def infer_action(
        ...
        inputs_embeds = self._forward_siglip_and_text_embedding(input_ids, pixel_values, intrinsic)
