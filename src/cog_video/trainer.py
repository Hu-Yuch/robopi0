...
class Trainer:
    ...
    def prepare_models(self) -> None:
        ...
        if hasattr(self.components.transformer,'lvm_transformer'):
            self.state.transformer_config = self.components.transformer.lvm_transformer.config
        else:
            self.state.transformer_config = self.components.transformer.config

        logger.info(f"Models config:{self.state.transformer_config}")
    ...
    def train(self) -> None:
        ...
        info = {
            ...
            "action trainable parameters": self.state.num_action_trainable_parameters,
            "lvm trainable parameters": self.state.num_lvm_trainable_parameters,
        ...
        for epoch in range(first_epoch, self.args.train_epochs):
            ...
            for step, batch in enumerate(self.data_loader):
                ...
                with accelerator.accumulate(models_to_accumulate):
                    ...
                    loss_video = self.compute_loss(batch)
                    if self.args.train_mode == "action":
                        loss_action = self.compute_action_loss(batch)
                        loss = loss_video*0.1 + loss_action*0.9
                    else:
                        loss = loss_video
    ...
    def __maybe_save_checkpoint(self, global_step: int, must_save: bool = False):
        if self.accelerator.distributed_type == DistributedType.DEEPSPEED or self.accelerator.is_main_process:
            if must_save or global_step % self.args.checkpointing_steps == 0:
                ...
                if self.accelerator.is_main_process:
                    torch.save(self.components.transformer.state_dict(),f'{save_path}/torch_model.pt')
