...
from src.rollout.rollout_video import RolloutVideo

from src.model.vla.pizero_robo_2 import PiZeroRoboInference
...
def rollout(env, model, task_oracle, cfg, subtask, lang_embeddings, val_annotations, record=False, rollout_video=None):
    ...
    goal = [val_annotations[subtask][0]]
    model.reset()
    start_info = env.get_info()

    for step in range(cfg.ep_len):
        inputs = model.preprocess_batch(obs, goal, split_mask=False,
...
@hydra.main(config_path="../config/eval", config_name="robobrain_calvin_2")
def main(cfg):
    ...
    dtype = torch.bfloat16 if cfg.get("use_bf16", False) else torch.float32
    #c = []
    #hydra.initialize(config_path="../../conf")
    #hydra.main(config_name="config_abc.yaml")(lambda x: c.append(x))()
    model = PiZeroRoboInference(cfg, use_ddp=False)
    data = torch.load(cfg.test_checkpoint_path, weights_only=True, map_location="cpu")
    data["model"] = {
        k.replace("_orig_mod.", ""): v for k, v in data["model"].items()
    }  # remove "_orig_mod." prefix if saved model was compiled
    model.load_state_dict(data["model"], strict=True)
    print(f"Loaded model from {cfg.test_checkpoint_path}")
    ...
    model.eval()
    if log_wandb:
        log_dir = get_log_dir(cfg.eval_fold)
