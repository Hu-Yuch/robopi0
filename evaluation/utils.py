...
from src.wrappers.hulc_wrapper import HulcWrapper
from src.utils.utils import add_text, format_sftp_path
...
def get_default_beso_and_env(train_folder, dataset_path, env=None, lang_embeddings=None, device_id=0, eval_cfg_overwrite={},cfg=None):
    #train_cfg_path = Path(train_folder) / ".hydra/config.yaml"
    #train_cfg_path = format_sftp_path(train_cfg_path)
    ...
    if env is None:
        rollout_cfg = OmegaConf.load(Path(__file__).parents[1] / "config/callbacks/rollout/default.yaml")
        #env = hydra.utils.instantiate(rollout_cfg.env_cfg, dataset, device, show_gui=False)
        env = HulcWrapper(dataset, device, show_gui=False)
